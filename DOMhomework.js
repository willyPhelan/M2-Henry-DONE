...
toDoText.setAttribute('id', index) ; // index sin comillas para que se lo pase por arg
if (todo.complete) { 
...
function buildToDos(toDos) {
  // Tu código acá:
let nuevoarray = toDos.map(function(todo,index) { // .Map recorre el areglo y aplica una fn que recibe como parametro, primero el elemento donde aplicara dicha fn y segundo el indice (la posicion en el array de dicho elemento)
  return buildToDo(todo, index) ;  // devuele la ejecucion de la fn buildToDo
})
return nuevoarray ; 
}
...
//Seleccionr el elemento cuyo id es 'toDoContainer' y almacenarlo en una variable denominada 'toDoContainer'
...
function displayToDos() {
  ...
  let paiterar = buildToDo(toDoItems) ; 
  for (let i = 0 ; i < paiterar.length ; i++) { 
    toDoContainer.appendChild(paiterar[i]) ; 
  }
